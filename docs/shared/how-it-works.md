...
|---|---|---|---|
| **ReaderFactory** | `reader_factory.py` | Selects `PN532Driver`, `PN7160Driver`, or UID-only `RC522Driver` from `reader_type`, validates reader-specific bus defaults | Tag parsing, gate policy, Happy Hare |
...
| **RC522Driver** | `rc522_driver.py` | RC522 SPI register protocol and UID-only ISO14443A reads | Rich tag parsing, Spoolman, gate policy, Happy Hare |
