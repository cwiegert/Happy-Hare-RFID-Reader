...
The event macros are in `~/printer_data/config/nfc/nfc_macros.cfg`. This file is read-only because it defines the NFC reader's interface to Happy Hare. To customize it, make a copy and include the copy instead of the original.

For lane readers, the Happy Hare-facing gate assignment commands live in
`nfc_macros.cfg` so the interface remains visible without putting it in Python.
