...
| Full-travel NFC homing, no UID | Optional warning when warning console mirroring is enabled, followed by the normal no-tag rewind message. | `WARNING  [<n>]: continuous scan reached max distance with no UID — rewinding` |
...
| Level-3 tag detail | No extra console message. | `INFO     nfc_gate: [shared] shared CHANGED — spool=<spool> uid=<uid> auto_created=<bool>; UID polling continues while staged` at `debug: 3` |
...
| Repeated staged UID (`debug: 3`) | No extra console message. | `DEBUG    nfc_gate: [shared] shared pending UID probe matched <uid>; rich read skipped` |
| Different UID while pending | No immediate console message; a successful full read emits the normal staged-tag success message. | `INFO     nfc_gate: [shared] shared UID changed from <old> to <new>; clearing staged data before rich read` |
