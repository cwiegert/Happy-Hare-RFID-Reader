...
| Stopped scan-jog started | `[SCAN] NFC[laneN]: stopped scan-jog started for gate <n> (max=<mm>mm  chunk=<mm>mm  substep=<mm>mm  reads=<n>  poll=<seconds>s)` | Same message at `INFO`; debug logs include the stopped-mode chunk/substep/read settings |
| Continuous scan-jog started | `[SCAN] NFC[laneN]: continuous scan-jog started for gate <n> (max=<mm>mm  step=<mm>mm  speed=<mm/s>  accel=<mm/s^2>  poll=<seconds>s)` | Same message at `INFO`; debug logs include the continuous-mode step/speed/accel/poll settings |
